import { candidateClaimSchema, } from "./domain.js";
function manifestDependencies(sources) {
...
function evidenceFor(source, pattern, authority) {
...
function collectEvidence(sources, pattern, authority, filter = () => true) {
...
function packageEvidence(sources, dependencyPattern) {
    return collectEvidence(sources, dependencyPattern, () => 0.64, (source) => /(?:^|\/)package\.json$/u.test(source.path));
}
function moduleImportEvidence(sources, modulePattern, authority = 0.96) {
    return sources.flatMap((source) => {
...
export function extractDeterministicClaims(sources) {
    ...
    const tsEvidence = collectEvidence(sources, /"(?:typescript|jsx|moduleResolution|compilerOptions)"|typescript/iu, (source) => (/(?:^|\/)tsconfig(?:\..+)?\.json$/u.test(source.path) ? 0.98 : 0.7), (source) => /tsconfig|package\.json$/u.test(source.path));
    ...
    const nextActive = [
        ...
        ...collectEvidence(sources, /\bnextConfig\b/u, () => 0.94, (source) => activeSource(source) && /(?:^|\/)next\.config\./u.test(source.path)),
    ];
    ...
    const githubOAuthEvidence = collectEvidence(sources, /(?:fetch|new URL)\(\s*["']https:\/\/github\.com\/login\/oauth\/(?:authorize|access_token)/u, () => 0.97, activeSource);
    ...
    for (const source of sources.filter((item) => item.kind === "documentation" && /(?:^|\/)README\.md$/iu.test(item.path))) {
    ...
    const sqliteEvidence = moduleImportEvidence(sources, /["'](?:better-sqlite3|drizzle-orm\/sqlite-core)["']/u, 0.92);
    ...
    const vitestEvidence = collectEvidence(sources, /(?:vitest|defineConfig\()/u, (source) => (source.path.includes("vitest") ? 0.94 : 0.65), (source) => /vitest|package\.json$/u.test(source.path));
