import { openHarikosDatabase } from "@harikos/db";
import { projectSnapshotSchema, projectTruthClaimSchema, } from "./domain.js";
...
function previousTruths(store, projectId) {
    return store.claims.listByProject(projectId).map((claim) => {
        const evidence = store.evidence.listByClaim(claim.id).flatMap((item) => {
...
function persistSources(store, projectId, sources) {
    const sourceIds = new Map();
    for (const source of sources) {
        const existing = store.sources.findByIdentity(projectId, source.kind, source.path, source.contentHash);
...
function persistResolution(store, projectId, resolution, sourceIds) {
    const existingClaims = new Map(store.claims.listByProject(projectId).map((claim) => [claim.id, claim]));
    ...
    for (const contradiction of resolution.contradictions) {
        if (existingContradictions.some((item) => item.claimAId === contradiction.claimAId &&
    ...
    for (const change of resolution.changes) {
        if (existingEvents.some((event) => event.type === "truth_changed" &&
...
export async function analyzeRepository(source, previous = [], options = {}) {
...
export async function scanAndPersistLocalProject(projectRoot, options = {}) {
    ...
    try {
        ...
        const resolution = resolveTruth(previousTruths(store, initialized.project.id), candidates, scannedAt, metadata.headSha);
