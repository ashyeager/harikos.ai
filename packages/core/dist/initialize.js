...
function readExistingConfig(configPath) {
    ...
    try {
        ...
        throw new InitializationError("INVALID_CONFIG", `HARIKOS config '${configPath}' is invalid. Fix or remove it before retrying initialization.`, { cause: error });
    }
}
...
function ensureGitIgnore(projectRoot) {
    ...
    writeFileSync(gitIgnorePath, `${existing}${separator}${HARIKOS_GITIGNORE_ENTRY}\n`, "utf8");
    return true;
}
export function initializeProject(options = {}) {
    ...
    if (existingConfig && existingConfig.projectRoot !== projectRoot) {
        throw new InitializationError("STATE_PROJECT_MISMATCH", `HARIKOS state belongs to '${existingConfig.projectRoot}', not '${projectRoot}'.`);
    }
    ...
    try {
        ...
        if (existingConfig && existingConfig.projectId !== project.id) {
            throw new InitializationError("STATE_PROJECT_MISMATCH", `HARIKOS config references project '${existingConfig.projectId}', but the database registered '${project.id}'.`);
        }
