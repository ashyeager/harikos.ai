...
import { z } from "zod";
import { changedFileSchema, repositoryCommitSchema, repositoryEntrySchema, repositoryFileSchema, repositoryMetadataSchema, } from "./domain.js";
...
export class LocalRepositorySource {
    ...
    async getMetadata() {
        ...
        const defaultBranch = runGit(this.root, ["branch", "--show-current"]) || "HEAD";
        const remote = spawnSync("git", ["-C", this.root, "remote", "get-url", "origin"], { encoding: "utf8", windowsHide: true });
        const remoteUrl = remote.status === 0 ? remote.stdout.trim() : "";
        const githubMatch = remoteUrl.match(/github\.com[/:](?<owner>[^/]+)\/(?<name>[^/]+?)(?:\.git)?$/iu);
    ...
    async getFiles(paths, ref) {
        const results = await Promise.allSettled(paths.map((path) => this.getFile(path, ref)));
        ...
        return output.split(/\r?\n/u).flatMap((line) => {
            ...
            return [
                changedFileSchema.parse({
                    path,
                    status,
                    previousPath: status === "renamed" && firstPath
...
export class GitHubRepositorySource {
    ...
    async getMetadata() {
        const repository = githubRepositorySchema.parse(await this.request(`/repos/${this.slug}`));
    ...
    async getTree(ref = "HEAD") {
        const response = githubTreeSchema.parse(await this.request(`/repos/${this.slug}/git/trees/${encodeURIComponent(ref)}?recursive=1`));
        if (response.truncated) {
            throw new Error("GitHub returned a truncated repository tree; HARIKOS will not treat it as complete evidence.");
        }
        return response.tree
            .map((entry) => repositoryEntrySchema.parse({
    ...
    async getFile(path, ref = "HEAD") {
        ...
        const response = githubContentSchema.parse(await this.request(`/repos/${this.slug}/contents/${normalized.split("/").map(encodeURIComponent).join("/")}?ref=${encodeURIComponent(ref)}`));
    ...
    async getFiles(paths, ref) {
        ...
        for (let index = 0; index < paths.length; index += 8) {
            const results = await Promise.allSettled(paths.slice(index, index + 8).map((path) => this.getFile(path, ref)));
            files.push(...results.flatMap((result) => result.status === "fulfilled" ? [result.value] : []));
        }
        return files;
    }
    async getChangedFiles(base, head) {
        const response = githubCompareSchema.parse(await this.request(`/repos/${this.slug}/compare/${encodeURIComponent(base)}...${encodeURIComponent(head)}`));
    ...
    async getCommit(ref = "HEAD") {
        const response = githubCommitSchema.parse(await this.request(`/repos/${this.slug}/commits/${encodeURIComponent(ref)}`));
        return repositoryCommitSchema.parse({
            sha: response.sha,
