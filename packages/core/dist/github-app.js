...
export const githubAppConfigSchema = z.object({
    ...
    privateKey: z.string().trim().refine((value) => value.startsWith("-----BEGIN ") && value.includes(" PRIVATE KEY-----"), { message: "A PEM encoded GitHub App private key is required." }),
...
export function readGitHubAppConfig(environment = process.env) {
...
export function createGitHubAppJwt(config, now = Math.floor(Date.now() / 1000)) {
...
export async function createGitHubInstallationToken(config, installationId, options = {}) {
    ...
    const response = await (options.fetcher ?? fetch)(`https://api.github.com/app/installations/${installationId}/access_tokens`, {
        method: "POST",
        headers: {
...
export async function getGitHubInstallation(config, installationId, fetcher = fetch) {
    if (!/^\d+$/u.test(installationId)) {
        throw new Error("A numeric GitHub installation ID is required.");
    }
    ...
    if (!response.ok) {
        throw new Error(`GitHub App installation lookup failed with status ${response.status}.`);
    }
    return githubInstallationSchema.parse(await response.json());
}
export async function listGitHubInstallationRepositories(config, installationId, fetcher = fetch) {
    const { token } = await createGitHubInstallationToken(config, installationId, {
        fetcher,
    });
    const repositories = [];
    for (let page = 1; page <= 10; page += 1) {
        const response = await fetcher(`https://api.github.com/installation/repositories?per_page=100&page=${page}`, {
            headers: {
                Accept: "application/vnd.github+json",
        ...
        if (!response.ok) {
            throw new Error(`GitHub installation repository lookup failed with status ${response.status}.`);
        }
        const pageData = installationRepositoriesSchema.parse(await response.json());
        repositories.push(...pageData.repositories);
        if (pageData.repositories.length < 100)
            break;
    }
    return repositories;
}
