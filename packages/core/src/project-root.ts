...
export function findProjectRoot(startPath = process.cwd()): string {
  ...
  try {
    ...
    throw new ProjectRootError(
      "INVALID_START_PATH",
      `Cannot inspect '${startPath}' because it does not exist or is inaccessible.`,
      { cause: error },
    );
  }
  ...
  if (result.error) {
    ...
    if (nodeError.code === "ENOENT") {
      throw new ProjectRootError(
        "GIT_UNAVAILABLE",
        "Git is required to initialize HARIKOS but was not found on PATH.",
        { cause: result.error },
      );
    }
    ...
    throw new ProjectRootError(
      "NOT_GIT_REPOSITORY",
      `Git could not inspect '${workingDirectory}'.`,
      { cause: result.error },
    );
  }
  ...
  if (result.status !== 0) {
    throw new ProjectRootError(
      "NOT_GIT_REPOSITORY",
      `'${workingDirectory}' is not inside a Git repository.`,
    );
  }
  ...
  try {
    ...
    throw new ProjectRootError(
      "INVALID_GIT_ROOT",
      `Git reported an inaccessible repository root: '${reportedRoot}'.`,
      { cause: error },
    );
  }
}
