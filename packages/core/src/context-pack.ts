...
function relevance(claim: ProjectTruthClaim, words: Set<string>): number {
...
export function composeContextPack(
  ...
  if (
    truths.some((claim) => claim.value === "Supabase Auth") &&
...
export function explainProjectTruth(
  snapshot: ProjectSnapshot,
  question: string,
  mode: "simple" | "technical" | "evidence" = "simple",
): string {
  ...
  if (mode === "technical") {
    return matches
      .map(
        (claim) =>
          `${claim.subject} uses ${claim.value} in ${claim.scope ?? "the project"} (${Math.round(claim.confidence * 100)}% confidence, ${claim.evidence.length} evidence sources).`,
      )
      .join(" ");
  }
