...
export default async function ProjectOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell snapshot={snapshot}>
      ...
      {/* METRICS ROW */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-line border border-line mb-10 rounded-sm overflow-hidden">
        <div className="bg-ink p-5 flex flex-col justify-center">
          <div className="flex items-center gap-2 mb-3">
            <span className="w-2 h-2 rounded-full bg-green shadow-[0_0_0_4px_rgba(16,185,129,0.1)] flex-shrink-0 animate-pulse"></span>
            <span className="font-mono text-[9px] tracking-widest text-muted uppercase">STATUS</span>
          </div>
          <strong className="text-white text-sm">Project Truth is current</strong>
          <span className="text-[10px] text-muted mt-1 font-mono">
            {new Date(snapshot.scannedAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
          </span>
        </div>
        <div className="bg-ink p-5 flex flex-col justify-center">
          <div className="flex items-center gap-2 mb-3">
