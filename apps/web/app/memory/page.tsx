...
export default function MemoryMarketingPage() {
  return <MarketingShell><main className="public-page memory-page">
    ...
    <PublicSection eyebrow="AGENT CONTINUITY" title={<>The next agent inherits<br /><span>the hard-won parts.</span></>} copy="A related task receives current Truth plus the prior constraint, failed attempt, decision, outcome, and relevant files." tone="accent"><div id="handoff"><ExampleLabel /><AgentHandoff /></div></PublicSection>
    <PublicSection eyebrow="MEMORY CONTROL" title={<>Useful history stays<br /><span>inspectable and reversible.</span></>} copy="Records remain visible to the user and can move through simple active, superseded, and archived states." tone="deeper"><div className="memory-control-grid">{[["ACTIVE", "Still useful for current work", "green"], ["SUPERSEDED", "Replaced but retained as history", "gray"], ["ARCHIVED", "Hidden from normal retrieval", "accent"]].map(([state, copy, tone]) => <article className={`tone-${tone}`} key={state}><i /><strong>{state}</strong><p>{copy}</p></article>)}</div></PublicSection>
