...
export function SideNav({ projectId }: { projectId: string }) {
  ...
  return <nav className="side-nav" aria-label="Project navigation"><span className="nav-label">PROJECT</span>{items.map(([label, icon, suffix]) => { const href = `${base}${suffix}`; const active = suffix ? pathname.startsWith(href) : pathname === href; return <Link aria-current={active ? "page" : undefined} className={active ? "active" : ""} href={href} key={label}><i>{icon}</i><span>{label}</span></Link>; })}</nav>;
}
