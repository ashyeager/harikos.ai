...
export function ThemeToggle() {
  ...
  return <span className="tooltip-root"><button aria-describedby="theme-toggle-tip" aria-label={`Color theme: ${value.preference === "light" ? "white" : "black"}. Switch to ${next === "light" ? "white" : "black"}.`} className="theme-toggle" onClick={() => value.setPreference(next)} type="button"><span aria-hidden="true">{value.preference === "light" ? "◐" : "◑"}</span><small>{value.preference === "light" ? "WHITE" : "BLACK"}</small></button><span className="tooltip-content" id="theme-toggle-tip" role="tooltip">Use {next === "light" ? "white" : "black"} theme</span></span>;
}
