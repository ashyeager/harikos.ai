...
export function AppChrome({ project, children }: { project: AppProjectSummary | undefined; children: ReactNode }) {
  ...
  return <div className={`app-frame ${menuOpen ? "mobile-nav-open" : ""}`}>
    ...
    <div className="app-workspace"><header className="app-topbar"><button aria-controls="app-navigation" aria-expanded={menuOpen} aria-label={menuOpen ? "Close navigation" : "Open navigation"} className="app-menu-button" onClick={() => setMenuOpen((value) => !value)} type="button"><i /><i /></button><div className={`scan-health ${project?.openContradictions || project?.refreshRequiredAt ? "has-attention" : ""}`}><i />{project ? <><strong>{project.refreshRequiredAt ? "REPOSITORY CHANGED" : project.openContradictions ? `${project.openContradictions} OPEN CONTRADICTION${project.openContradictions === 1 ? "" : "S"}` : "LAST SCAN COMPLETE"}</strong><span>{project.refreshRequiredAt ? "MANUAL VERIFICATION REQUIRED" : `${formatUtcDateTime(project.scannedAt)} / ${project.sha.slice(0, 8)}`}</span></> : <><strong>WORKSPACE</strong><span>SELECT OR CONNECT A PROJECT</span></>}</div><div className="topbar-actions"><CommandPalette projectId={project?.id} />{project ? <Link className="topbar-context" href={`/app/project/${project.id}/context`}>Prepare context <span>&rarr;</span></Link> : <Link className="topbar-context" href="/app/projects">Projects <span>&rarr;</span></Link>}</div></header><main className="app-content" id="main-content" tabIndex={-1}>{children}</main></div>
  </div>;
}
