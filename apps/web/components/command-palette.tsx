...
export function CommandPalette({ projectId }: { projectId?: string }) {
  ...
  return <><button aria-label="Open command palette" className="command-trigger" onClick={() => setOpen(true)} ref={triggerRef} type="button"><span>COMMANDS</span><kbd>CTRL K</kbd></button>{open ? <div className="command-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) closePalette(); }} role="presentation"><section aria-label="Command palette" aria-modal="true" className="command-palette" onKeyDown={trapFocus} ref={dialogRef} role="dialog"><header><span>&gt;_</span><input autoFocus aria-label="Filter commands" onChange={(event) => setQuery(event.target.value)} placeholder="Go to a page…" value={query} /><kbd>ESC</kbd></header><div>{items.length ? items.map(([label, href, shortcut]) => <Link href={href} key={href} onClick={closePalette}><i>{shortcut}</i><strong>{label}</strong><span>&rarr;</span></Link>) : <p>No matching command.</p>}</div><footer><span>Navigate</span><span>Enter to open</span><span>Esc to close</span></footer></section></div> : null}</>;
}
