...
export function BillingActions({ enabled }: { enabled: boolean }) {
  ...
  return <div className="billing-actions"><button className="button button-dark" disabled={!enabled || pending !== undefined} onClick={() => open("checkout")} type="button">{pending === "checkout" ? "Opening checkout..." : "Start Pro trial"} <span>&rarr;</span></button><button className="button button-ghost" disabled={!enabled || pending !== undefined} onClick={() => open("portal")} type="button">{pending === "portal" ? "Opening portal..." : "Manage billing"}</button>{!enabled ? <p className="empty-state">Billing is unavailable for this deployment. No upgrade state is being simulated.</p> : null}{error ? <p className="inline-error" role="alert">{error}</p> : null}</div>;
}
